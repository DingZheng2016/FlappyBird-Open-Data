cc.Class({
    ...
    start: function() {
        ...
        if (CC_WECHATGAME) {
            window.wx.onMessage(data => {
                ...
                } else if (data.messageType == 4) {//获取本人最高分
                    this.gainScore(data.MAIN_MENU_NUM);
    ...
    submitScore: function(MAIN_MENU_NUM, score) { //提交得分
        if (CC_WECHATGAME) {
            console.log('Sub: submit score ' + score + ' ' + this.myscore);
            if(score > this.myscore){
    ...
    gainScore: function(MAIN_MENU_NUM){
        let self = this;
        this.myscore = 0;
        if (CC_WECHATGAME) {
            console.log('Sub: gain score');
            window.wx.getUserCloudStorage({
                ...
                success: function (getres) {
                    ...
                    if (getres.KVDataList.length != 0)
                        self.myscore = getres.KVDataList[0].value;
                    console.log('myscore: ' + self.myscore);
                },
                fail: function (res) {
